...
#define WHEEL_SEPARATION  0.2771//	/* m */0.252, 0.2629 0.2528
#define E_d 1.0057// 1.01
...
int main()
{
  ...
  double waypoints [4][2]={{1, 0},{1,1},{0, 1},{0,0}};
...
*/
   ...
   switch (mission.state) {
    case ms_init:
       ...
       mission.state= ms_goto_setup;//ms_fwd;//ms_follow_line;//      
    ...
    case ms_turn:
      if (turn(angle,mot.t_speed_t,mission.time)){
         n=n-1;
      if (n==0) 
	      mission.state=ms_end;
	    else
...
/*void update_map(){
  //move to the main or a strcut or sth
  double cell=0.2, map_xsize=5.0, map_ysize=5.0, R_safe=0.4,laser_max;
  int map[map_xsize/cell][map_ysize/cell];
  int x,y;//current
  int xg,yg;//goal
  int xcg, ycg;//closest accesible point to the goal. 
  double alpha;
  //if it returns 1000 then laser_max
  
  //MAP: -1=unkown, positive=known and open; even = accesible; x3= visited; x5=current closest accesible to the goal.
  for(int sign=-1;sign<2; sign=sign+2){
    for(int i=0;i<laser_max/cell;i++){
      for(int j=0;j<laser_max/cell;j++){
        for(int n=0;n<9;n++){
          
          if(map[x+i*sign][y+j*sign]<0){
            if(atan2(y+j*sign,x+i*sign)<alpha*n+theta-M_PI/2 && atan2(y+j*sign,x+i*sign)>alpha*(n+1)+theta-M_PI/2){
              if(sqrt(i*i+j*j)*cell<laser[n]){
                map[x+i*sign][y+j*sign]*=-2;//*-1 to say it was discovered and in the map. *2 to put it in the accesible points
                if (pow(xg-x+i*sign,2)+pow(yg-y+j*sign,2)<((xg-xcg)*(xg-xcg)+(xg-xcg)*(xg-xcg))){
                  map[x+i*sign][y+j*sign]=(map[x+i*sign][y+j*sign])*5;
                  map[xcg][ycg]=map[xcg][ycg]/5;
                  xcg=x+i*sign;
                  ycg=y+j*sign;
                }
              }
              else if(sqrt(i*i+j*j)<laser_max){
                //neighnbourghs
                for (int k=0;k<2;k++){
                  if(map[x+i*sign+k][y+j*sign]%2==0)
                   map[x+i*sign+k][y+j*sign]/=2;//so it doesn belong to the accesible points anymore
                  if(map[x+i*sign-k][y+j*sign]%2==0)
                   map[x+i*sign-k][y+j*sign]/=2;
                   
                  if(map[x+i*sign][y+j*sign+k]%2==0)
                   map[x+i*sign][y+j*sign+k]/=2;//so it doesn belong to the accesible points anymore
                  if(map[x+i*sign][y+j*sign-k]%2==0)
                   map[x+i*sign][y+j*sign-k]/=2;
                   
                }
                
                if(map[x+i*sign-1][y+j*sign-1]%2==0)
                   map[x+i*sign-1][y+j*sign-1]/=2;
                if(map[x+i*sign-1][y+j*sign+1]%2==0)
                   map[x+i*sign-1][y+j*sign+1]/=2;
                if(map[x+i*sign+1][y+j*sign-1]%2==0)
                   map[x+i*sign+1][y+j*sign-1]/=2;
